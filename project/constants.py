...
class Constants(object):
    ...
    def __init__(self):
        ...
        self.GUI_SERVER_ADDR_FULL = (
            f"{self.GUI_SERVER_ADDR}:{int(self.GUI_SERVER_PORT)}"
        )

        # WebLogger Server
        self.WEBLOGGER_SERVER_PORT = 5002
        self.WEBLOGGER_SERVER_ADDR = "localhost"
        self.WEBLOGGER_SERVER_ADDR_FULL = (
            f"{self.WEBLOGGER_SERVER_ADDR}:{int(self.WEBLOGGER_SERVER_PORT)}"
        )

        # AgentsCheckup Server
        self.AGENTSCHECKUP_SERVER_PORT = 5003
        self.AGENTSCHECKUP_SERVER_ADDR = "localhost"
        self.AGENTSCHECKUP_SERVER_ADDR_FULL = (
            f"{self.AGENTSCHECKUP_SERVER_ADDR}:{int(self.AGENTSCHECKUP_SERVER_PORT)}"
        )
